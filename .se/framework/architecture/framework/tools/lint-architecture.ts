...
function validateProjectArchitecture(projectArchitectureRoot: string, label: string) {
  for (const file of projectRequiredFiles) {
    if (!fs.existsSync(path.join(projectArchitectureRoot, file))) {
      errors.push(`missing required file: ${label}/${file}`);
    }
  }

  const projectArchitecture = path.join(projectArchitectureRoot, 'ARCHITECTURE.md');
  if (fs.existsSync(projectArchitecture)) {
    const markdown = readText(projectArchitecture);
    for (const heading of projectRequiredHeadings) {
      if (!hasHeading(markdown, heading)) {
        errors.push(`${label}/ARCHITECTURE.md missing heading: ${heading}`);
      }
    }
  }

  const modulesDir = path.join(projectArchitectureRoot, 'modules');
  if (!fs.existsSync(modulesDir)) {
    errors.push(`missing required directory: ${label}/modules`);
  } else {
    ...
    for (const file of moduleFiles) {
      ...
      for (const heading of moduleRequiredHeadings) {
        if (!hasHeading(markdown, heading)) {
          errors.push(`${label}/modules/${file} missing heading: ${heading}`);
        }
      }
    }
  }
...
const seProjectArchitectureRoot = path.join(root, '.se', 'project', 'architecture');
if (fs.existsSync(seProjectArchitectureRoot)) {
  validateProjectArchitecture(seProjectArchitectureRoot, '.se/project/architecture');
}

const legacyProjectRoot = path.join(architectureRoot, 'project');
if (fs.existsSync(legacyProjectRoot)) {
  validateProjectArchitecture(legacyProjectRoot, 'se.architecture/project');
}
